...
def img_code():
    myqr.run(
        words='https://yfuean-student-manager.oss-cn-shanghai.aliyuncs.com/img/avatar.jpg',
        # 设置容错率为最高
        version=1,
        # 控制纠错水平，范围是l、M、Q、H，从左到右依次升高
        level='H',
        # 背景图
        picture='./res/img/bg5.jpeg',
        # 彩色二维码
        colorized=True,
        ...
        brightness=1.0,
        # 保存文件名字
        save_name='code2.jpeg',
        # 保存位置
        save_dir=os.getcwd()+'/res/img/')


def draw():
    img = Image.open('./res/img/code2.jpeg')
    w, h = img.size
    txt = '云上的日子'
